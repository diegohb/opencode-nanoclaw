...
**通过隔离保障安全:** 智能体运行在 Linux 容器（Docker）中。它们只能看到被明确挂载的内容。即便通过 Bash 访问也十分安全，因为所有命令都在容器内执行，不会直接操作您的宿主机。
...
- **网络访问** - 搜索和抓取网页内容
- **容器隔离** - 智能体在 Docker (macOS/Linux) 的沙箱中运行
...
Docker 提供跨平台支持（macOS 和 Linux）和成熟的生态系统。
