...
Provider credentials are managed by the OneCLI gateway and injected at request time. Anthropic or OpenRouter access is configured on the host; containers should not depend on local Claude credential files or `claude setup-token`. Run `onecli --help` if you need to inspect or update the current credential setup.
...
- `/workspace/opencode-state` - Persisted OpenCode state for this group
...
You can read and write to `/workspace/global/AGENTS.md` for facts that should apply to all groups. Only update global memory when explicitly asked to "remember this globally" or similar.
