...
const NANOCLAW_HOST =
  process.env.NANOCLAW_HOST || 'http://host.docker.internal:3979';
...
class NanoClawTeamsHandler extends TeamsActivityHandler {
  constructor() {
    ...
    this.onMessage(async (context: TurnContext) => {
      ...
      // Forward to NanoClaw host inbound server
      try {
        ...
        await adapter.process(
          {
            ...
            method: req.method!,
            url: req.url!,
          } as Parameters<typeof adapter.process>[0],
          {
            status: (code: number) => ({
              send: (b: unknown) => {
                res.writeHead(code);
          ...
          } as Parameters<typeof adapter.process>[1],
  ...
  if (url === '/send' && method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        const conversationId = (jid as string).replace(/^teams:/, '');
