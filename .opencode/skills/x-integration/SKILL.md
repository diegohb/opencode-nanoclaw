...
npx dotenv -e .env -- npx tsx .opencode/skills/x-integration/scripts/setup.ts
# Verify: data/x-auth.json should exist after successful login

# 2. Rebuild container to include skill
./container/build.sh
# Verify: Output shows "COPY .opencode/skills/x-integration/agent.ts"
...
```typescript
import { handleXIpc } from '../.opencode/skills/x-integration/host.js';
```
...
```typescript
// @ts-ignore - Copied during Docker build from .opencode/skills/x-integration/
...
Change build context from `container/` to project root (required to access `.opencode/skills/`):
```bash
# Find:
...
First, update the build context paths (required to access `.opencode/skills/` from project root):
```dockerfile
# Find:
...
COPY .opencode/skills/x-integration/agent.ts ./src/skills/x-integration/
```
...
```bash
echo '{"content":"Test tweet - please ignore"}' | npx dotenv -e .env -- npx tsx .opencode/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | npx dotenv -e .env -- npx tsx .opencode/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | npx tsx .opencode/skills/x-integration/scripts/post.ts
```
