...
**Workaround:** The system extracts only the authentication variable `ANTHROPIC_API_KEY` from `.env` and mounts it for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "API: ${#ANTHROPIC_API_KEY} chars"'
```
...
[ -f .env ] && grep -q "ANTHROPIC_API_KEY=sk-" .env && echo "OK" || echo "MISSING - add ANTHROPIC_API_KEY to .env"
