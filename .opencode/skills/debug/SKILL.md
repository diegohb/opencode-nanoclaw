...
src/container-runner.ts               container/agent-runner/
    │                                      │
  │ spawns container                      │ runs OpenCode
    │ with volume mounts                   │ with MCP servers
    │                                      │
    ├── data/env/env ──────────────> /workspace/env-dir/env
    ├── groups/{folder} ───────────> /workspace/group
    ├── data/ipc/{folder} ────────> /workspace/ipc
    ├── data/sessions/{folder}/opencode-state/ ──> /workspace/opencode-state (isolated per-group)
    └── (main only) project root ──> /workspace/project
```

  **Important:** OpenCode state must be mounted to `/workspace/opencode-state` for session resumption to work.
...
| **OpenCode state** | `data/sessions/{group}/opencode-state/` | Per-group OpenCode session state |
...
**Root cause:** OpenCode state is not being persisted to the mounted `/workspace/opencode-state` directory.

**Check the mount path:**
```bash
# In container-runner.ts, verify mount is to /workspace/opencode-state
grep -A3 "OpenCode state" src/container-runner.ts
```

**Verify state is accessible:**
```bash
docker run --rm --entrypoint /bin/bash \
  -v $(pwd)/data/sessions/test/opencode-state:/workspace/opencode-state \
  nanoclaw-agent:latest -c '
ls -la /workspace/opencode-state 2>&1 | head -5
'
```

**Fix:** Ensure `container-runner.ts` mounts to `/workspace/opencode-state`:
```typescript
mounts.push({
...
The agent-runner now writes `opencode.json`, sets `OPENCODE_PROJECT=/workspace/group`, and persists runtime state in `/workspace/opencode-state`.

When debugging, verify these current assumptions instead of older Claude SDK behavior:

```bash
grep -n "opencode-state\|OPENCODE_PROJECT\|opencode.json" src/container-runner.ts container/agent-runner/src/opencode-runner.ts
```
...
OpenCode state is stored per-group in `data/sessions/{group}/opencode-state/` for security isolation. Each group has its own state directory, preventing cross-group access to conversation history.

**Critical:** The mount target must be `/workspace/opencode-state`.
...
rm -rf data/sessions/{groupFolder}/opencode-state/
...
grep -q "/workspace/opencode-state" src/container-runner.ts 2>/dev/null && echo "OK" || echo "WRONG - should mount to /workspace/opencode-state"
