...
A personal OpenCode assistant with multi-channel support, persistent memory per conversation, scheduled tasks, and container-isolated agent execution.
...
│  │    • data/sessions/{group}/opencode-state/ → /workspace/opencode-state      │    │
...
| Agent | @opencode-ai/sdk | Run OpenCode with tools and MCP servers |
...
The core ships with no channels built in — each channel (WhatsApp, Telegram, Slack, Discord, Gmail) is installed as a NanoClaw skill that adds the channel code to your fork. Channels self-register at startup; installed channels with missing credentials emit a WARN log and are skipped.
...
To add a new channel, contribute a skill to `.opencode/skills/add-<name>/` that:
...
```
nanoclaw/
├── AGENTS.md                      # Project context for the agent runtime
├── docs/
...
│
├── container/
│   ├── Dockerfile                 # Container image (runs as 'node' user, includes OpenCode runtime)
...
│   ├── sessions/                  # Per-group OpenCode state
...
2. **Agent Context Loading**
   ...
   - OpenCode loads the instruction files declared in `opencode.json`:
...
2. Session ID is passed to the OpenCode session API
3. OpenCode continues the conversation with full context
4. Session transcripts are stored as JSONL files in `data/sessions/{group}/opencode-state/`
...
|------------|------------------|-------|
| OpenCode state | data/sessions/{group}/opencode-state/ | Per-group isolation, mounted to /workspace/opencode-state |
...
| "OpenCode process exited with code 1" | Session mount path wrong | Ensure mount is to `/workspace/opencode-state` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | OpenCode state not persisted | Ensure `data/sessions/{group}/opencode-state/` is mounted to `/workspace/opencode-state` |
