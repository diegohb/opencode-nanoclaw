...
export async function run(args: string[]): Promise<void> {
  ...
  const groupAgentsFilePath = path.join(
    projectRoot,
    'groups',
    parsed.folder,
    'AGENTS.md',
  );
  if (!fs.existsSync(groupAgentsFilePath)) {
    ...
    if (fs.existsSync(templatePath)) {
      fs.copyFileSync(templatePath, groupAgentsFilePath);
      logger.info(
        { file: groupAgentsFilePath, template: templatePath },
